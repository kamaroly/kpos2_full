...
class invoice_histories_model extends CI_Model {
	 ...
	 */
	function insert_history_note($invoice_id, $history_body, $clientcontacts_id = '', $contact_type = 1,$date_sent=false)
	{
		$historyInfo = array(
							'sale_id' => $invoice_id,
							'clientcontacts_id' => serialize($clientcontacts_id),
							'date_sent' => $date_sent?$date_sent:date("Y-m-d"),
	...
	function insert_note($invoice_id, $note,$date)
	{
		$this->insert_history_note($invoice_id, $note, '', 2,$date);
	}
}
?>