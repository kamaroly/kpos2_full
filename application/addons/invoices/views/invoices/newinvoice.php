...
lang_amount = new String("");
lang_status = new String("Status");
lang_quantity = new String("");
lang_work_description = new String(" ");
lang_taxable = new String("");
lang_amount = new String("");
</script>

<link type="text/css" rel="stylesheet" href="<?php echo base_url();?>css/calendar.css" />
<link type="text/css" rel="stylesheet" href="<?php echo base_url();?>css/invoice.css" />
...
<script type="text/javascript" src="<?php echo base_url();?>js/createinvoice.js"></script>
<script type="text/javascript">
<!--
...
function build_calendar()
{
	...
	str += '<table class="calendar table table-bordered table-striped invoice_items stripe span3" cellspacing="0" cellpadding="0" border="0" >';
	str += '<tr>';
	str += '<td class="calnavleft btn btn-small btn-primary" onClick="change_month(-1, \'' + this.id + '\')">&lt;&lt;<\/td>';
	str += '<td colspan="5" class="calheading well">' + months[this.month] + ' ' + this.year + '<\/td>';
	str += '<td class="calnavright btn btn-small btn-primary" onClick="change_month(1, \'' + this.id + '\')">&gt;&gt;<\/td>';
	str += '<\/tr>';
	...
	for (j = 0; j < 42; j++)
	{
		...
		{
			str += '<td id="" class="caldaycells" onClick="set_date(this,\'' + this.id + '\'); return false;"  onMouseOver="javascript:cell_highlight(this,\'' + displayNum + '\',\'' + this.id + '\');" onMouseOut="javascript:cell_reset(this,\'' + displayNum + '\',\'' + this.id + '\');" ><a href="#">' + displayNum + '<\/a><\/td>';
		}
...
<p><label>Item <input type="text" class="item" name="item" size="40" /></label> <label>Quantity <input type="text" class="quantity" name="quantity" size="5" value="1" onblur="recalculate_items();" /></label> <label>Amount $<input type="text" class="amount" name="amount" size="5" value="0.00" onblur="recalculate_items();" /></label></p>
</div>

<form action="<?php echo base_url();?>invoices/newinvoice" method="post" accept-charset="utf-8" id="createInvoice" name="my_form" autocomplete="off">

	
	  <label for="client_id"><?php echo $this->lang->line('invoice_select_client');?>
		...
		<?php echo $this->lang->line('invoice_number');?>
		 <input type="text" name="invoice_number" id="invoice_number" value="<?php echo ($this->validation->invoice_number) ? ($this->validation->invoice_number) : ($suggested_invoice_number);?>" />
	...
	<p id="dateIssuedContainer">
		<?php echo $this->lang->line('invoice_date_issued_full');?> 
		<input type="text" name="dateIssued" id="dateIssued" 
		value="<?php echo $invoiceDate; ?>"/></label>
		<?php echo $this->validation->dateIssued_error; ?>
	</p>	<div id="cal1Container" style="display: none;">
<script type="text/javascript">
		var entry_date = new calendar("entry_date", 1374746485, true);
		document.write(entry_date.write());
	</script></div>

<div class="work_description">
	<table class="invoice_items  table table-bordered table-striped invoice_items stripe">
		<thead>
		<tr>
			<th>Quantity</th>
		...
		<tbody id="item_area">
		<tr class="item_row">
			<td><input type="text" name="items[1][quantity]" size="3" value="1" onkeyup="recalculate_items();" />
			</td> 
			<td>
				
				<textarea name="items[1][work_description]" id="work_description" cols="20" rows="3"></textarea>
				
			</td>
			<td><input type="checkbox" name="items[1][taxable]" value="1" onclick="recalculate_items();" 
			checked="checked" /></td>
			<td nowrap="nowrap"><input type="text" id="amount" name="items[1][amount]" size="5" value="0.00" onkeyup="recalculate_items();" value="" />
			</td>
			<td>&nbsp;</td>
		</tr>
		</tbody>
	...
	<p class="button"  id="new_item"><a  href="#" onclick="return create_itemized_fields();" class="clientnew btn btn-small btn-primary">
	 <?php echo $this->lang->line('invoice_new_item');?></a>
	</p>
...
<?php if (!empty($row) AND $row->tax_status):?>
...
<?php endif;?>
	...
	<p>
	<input type="submit"   name="createInvoice" class="btn btn-small btn-primary" id="createInvoice"
	 value="<?php echo $this->lang->line('actions_create_invoice');?>" />
	</p>
</form>
 <!--  We are loading the footer of our page here-->
 <?=$this->load->view('theme/partials/footer');?>
