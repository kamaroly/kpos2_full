...
<body>
	...
	<table class="invoice_items stripe">
		...
		<tr valign="top">
			...
			<td><p><?php echo $this->settings_model->get_setting('currency_symbol',$this->user_id) . str_replace('.', $this->config->item('currency_decimal'), $item->amount);?> <?php if ($item->taxable == 0){echo '(' . $this->lang->line('invoice_not_taxable') . ')';}?></p></td>
			<td><p><?php echo $this->settings_model->get_setting('currency_symbol',$this->user_id) . number_format($item->quantity * $item->amount, 2, $this->config->item('currency_decimal'), '');?></p></td>
		</tr>
		<?php endforeach;?>
	</table>
	...
	<div id="footer">
		<?php if ($this->settings_model->get_setting('display_branding',$this->user_id) == 'y'):?>
			<p>
				<?php echo $this->lang->line('invoice_generated_by');?> 
				<?php echo $this->lang->line('bambooinvoice_logo');?><br />
				<a href="http://www.huguka.org/">http://www.huguka.org</a>
			</p>
		<?php endif;?>
	</div>
