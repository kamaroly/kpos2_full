   ...
   */
	...
	*/
	...
	function view($item_id=-1)
	{
		...
		$data['items_categories']=$this->Mdl_items_categories->get_categories(null,10000,0  )->result();
		$data['item_info']=$this->Item->get_info($item_id);
		$data['item_tax_info']=$this->Item_taxes->get_info($item_id);
		
		//Getting account types
		$data['accounts']  = $this->Myfinamodel->list_accounts(array(0=>$this->config->Item('foreign_cash_account'),
				                                                     1=>$this->config->Item('local_cash_account'),
				                                                     2=>$this->config->Item('insurance_parent_account')));
	...
	function inventory($item_id=-1)
	{
		...
		$data['item_category']=$this->Mdl_items_categories->get_categories($data['item_info']->category_id,1)->row()->cname;
	...
	function save($item_id=-1)
	{
		...
		'vat_type'=>$this->input->post('vat_type'), //only to be used in tz
		'quantity'=>$this->input->post('quantity'),
		'back_stock'=>$this->input->post('back_stock'),
		...
		if($this->Item->save($item_data,$item_id))
		{
			...
			$tax_names = $this->input->post('vat_type');

			$tax_percents = ($this->input->post('vat_type')=='B')?18:0; //@TODO SAVE THE ITEM TAXES IN THE DATABASE;
             //This is modified for the RRA
             $items_taxes_data[]=array('name'=>$tax_names, 'percent'=>$tax_percents );
             /*
	...
	function bulk_update()
	{
		...
		if(empty($item_data) || $this->Item->update_multiple($item_data,$items_to_update))
		{
			...
			$tax_percents = ($this->input->post('vat_type')=='B')?18:0; 
  ...
  function do_excel_import()
	{
		...
		else
		{
			if (($handle = fopen($_FILES['file_path']['tmp_name'], "r")) !== FALSE)
			{
				...
				{
					//Let's check if the category is in the database before we upload the item
					if(count($this->categories->exists_by_name($data[2]))==1)
					{
						//Yes category found in the databse
						 $category_id=$this->categories->exists_by_name($data[2])->id;
					}
					else
					 {
					 	//This category is not in our database let's create it.
					 	$category_data=array('cname'=>$data[2],
					 			             'Description'=>'No Description given');
					 	
					 	//Let's Create the category
					 	if($this->categories->save($category_data))
					 	{
					 		// now category is created let's get the id of the created category
					 	 $category_id=$category_id=$this->categories->exists_by_name($data[2])->id;
					 	}
                    ...
                    'expiration_date'=>	$data[16]?$data[16]:'',	
					...
					if ($item_number != "")
					{
						...
						$item_data['item_number']=preg_replace('/\s+/', '',addslashes($data[1]));
					}
