...
function  __construct()
	...
	function index()
	{
		redirect('/accounts/summary', 'location');
	}
	...
	function additem($cid="all")
	{
	    ...
	    { 
			...
			$data['body'] = $this->load->view('transaction/add_item', $data, TRUE);
		...
		{  
			...
			redirect('/transaction/summary/'.$cid, 'location');
	    }
	}
	...
	function edititem($tid,$cid)
	{
	    ...
	    else
		{
			...
			redirect('/transaction/summary/'.$table['tr_cid1'], 'location');
	...
	function addpredefined()
	{
	    ...
	    {//Something goes wrong..
	    	//Go back to the form
			$data['body'] = $this->load->view('transaction/add_predefined', $data, TRUE);
		    $this->load->view('page', $data);
	    }
	    else
		{//All seem to go  well
			...
			redirect('/transaction/listpredefined/', 'location');
	    }
	}
	...
	function editpredefined($pid)
	{
		...
		//Loading Models...
		$this->load->model('Myfinamodel', '', TRUE);
	    if (!$this->Myfinamodel->is_exist_predefined($pid)) { redirect('/transaction/listpredefined/', 'location');}
	...
	function summary($cid='all')
	{
		...
		$data['table'] = $this->Myfinamodel->list_transactions($cid,$data['tyear'],$data['tmonth']);
		...
		if ($cid == 'all') $data['body'] = $this->load->view('transaction/summary_all', $data, TRUE);
		//elseif ($data['ctype'] == 'b' && $data['cincome'] == 1) $data['body'] = $this->load->view('transaction/summary_budget_income', $data, TRUE);
		//elseif ($data['ctype'] == 'b' && $data['cincome'] == 0) $data['body'] = $this->load->view('transaction/summary_budget_spend', $data, TRUE);
		elseif ($data['ctype'] == 'b') $data['body'] = $this->load->view('transaction/summary_budget', $data, TRUE);
		else $data['body'] = $this->load->view('transaction/summary', $data, TRUE);
		$this->load->view('page', $data);
	}
	...
	function listpredefined()
	{
		...
		$data['body'] = $this->load->view('transaction/listpredefined', $data, TRUE);
		$this->load->view('page', $data);
	}
	...
	function deleteitem($tid, $redirect = 1)
	{
		...
		if($redirect == 1) {redirect($this->db_session->userdata('session_referer'), 'location');}
	}
    ...
    function _insert_transaction($data)
    {
		...
		//$tr_index = $this->Myfinamodel->highest_tr_index($data['tto'],$insert['tdate']) + 1;
