...
class MY_Model extends CI_Model
{
     ...
     */
    protected $primary_key = 'POA_CODE';
    ...
    /**
     * Fetch a single record based on an arbitrary WHERE call. Can be
     * any valid value to $this->_database->like().
     */
    public function get_like()
    {
    	...
    	if ($this->soft_delete && $this->_temporary_with_deleted !== TRUE)
    	{
    		$this->_database->like($this->soft_delete_key, FALSE);
    	}
    ...
    /**
     * Fetch an array of records based on an arbitrary OR LIKE call.
     */
    public function get_many_or_like()
    {
    	$where = func_get_args();
    	$this->_set_or_like($where);
    
    	if ($this->soft_delete && $this->_temporary_with_deleted !== TRUE)
    	{
    		$this->_database->or_like($this->soft_delete_key, FALSE);
    	}
    ...
    /**
     * Set OR LIKE parameters, cleverly
     */
    protected function _set_or_like($params)
    {
    	if (count($params) == 1)
    	{
    		$this->_database->or_like($params[0]);
    	}
    	else
    	{
    		$this->_database->or_like($params[0], $params[1]);
    	}
    }
