...
function get_person_data_row($person,$controller)
{
	...
	$table_data_row.='<td width="5%">'.anchor($controller_name."/view/$person->person_id/width:$width", $CI->lang->line('common_edit'),array('class'=>'thickbox btn btn-small btn-info','title'=>$CI->lang->line($controller_name.'_update'))).'</td>';		
	$table_data_row.='</tr>';
...
function get_supplier_manage_table($suppliers,$controller)
{
	...
	foreach($headers as $header)
	{
		$table.="<th class='primary'>$header</th>";
	}
	$table.='</tr></thead><tbody>';
...
function get_supplier_data_row($supplier,$controller)
{
	...
	$table_data_row.='<td width="5%">'.anchor($controller_name."/view/$supplier->person_id/width:$width", $CI->lang->line('common_edit'),array('class'=>'thickbox btn btn-small btn-info','title'=>$CI->lang->line($controller_name.'_update'))).'</td>';		
	$table_data_row.='</tr>';
...
/*
 Gets the html table to manage items.
*/
function get_items_categories_manage_table($items,$controller)
{
	$CI =& get_instance();
	$table='<table class="tablesorter" id="sortable_table">';

	$headers = array('<input type="checkbox" id="select_all" />',
			$CI->lang->line('items_category_id'),
			$CI->lang->line('items_category_name'),
			$CI->lang->line('items_category_description'),
			$CI->lang->line('items_action'),
	);
	...
	{
		if($header=='Action'):
		$table.="<th >$header</th>"; //if it's the action then take space of the columns
		else:
		$table.="<th >$header</th>";
		endif;
	}
	$table.='</tr></thead><tbody>';
	$table.=get_items_category_manage_table_data_rows($items,$controller);
	$table.='</tbody></table>';
	return $table;
}
 ...
 * @method to help in category tables management
 * @param unknown_type $items
 * @param unknown_type $controller
 * @return string
 */
function get_items_category_manage_table_data_rows($items,$controller)
{
	$CI =& get_instance();
	$table_data_rows='';
	...
	{
		$table_data_rows.=get_item_category_data_row($item,$controller);
	}

	if($items->num_rows()==0)
	{
		$table_data_rows.="<tr><td colspan='4'><div class='warning_message' style='padding:7px;'>".$CI->lang->line('items_no_items_to_display')."</div></tr></tr>";
	}
 ...
 * @return string
 */
function get_item_category_data_row($item,$controller)
{
	$CI =& get_instance();
	$controller_name=strtolower(get_class($CI));
	$width = $controller->get_form_width();
	...
	$table_data_row.='<td width="15%">'.$item->id.'</td>';
	$table_data_row.=$item->cname?'<td width="20%">'.$item->cname.'</td>':'';
	$table_data_row.='<td width="14%">'.$item->description.'</td>';
	$table_data_row.='<td width="5%">'.anchor($controller_name."/view/$item->id/width:$width", $CI->lang->line('common_edit'),array('class'=>'thickbox','title'=>$CI->lang->line($controller_name.'_update'))).'</td>';
...
function get_item_data_row($item,$controller)
{
	...
	$table_data_row.=isset($item->cname)?'<td width="14%">'.$item->cname.'</td>':'';
	...
	$table_data_row.='<td width="5%">'.anchor($controller_name."/view/$item->item_id/width:$width", $CI->lang->line('common_edit'),array('class'=>'thickbox btn btn-small btn-success','title'=>$CI->lang->line($controller_name.'_update'))).'</td>';		
	
	//Ramel Inventory Tracking
	$table_data_row.='<td width="10%">'.anchor($controller_name."/inventory/$item->item_id/width:$width", $CI->lang->line('common_inv'),array('class'=>'thickbox btn btn-small btn-inverse','title'=>$CI->lang->line($controller_name.'_count'))).'</td>';//inventory count	
	$table_data_row.='<td width="5%">'.anchor($controller_name."/count_details/$item->item_id/width:$width", $CI->lang->line('common_det'),array('class'=>'thickbox btn btn-small btn-info','title'=>$CI->lang->line($controller_name.'_details_count'))).'</td>';//inventory details	
...
function get_giftcard_data_row($giftcard,$controller)
{
	...
	$table_data_row.='<td width="5%">'.anchor($controller_name."/view/$giftcard->giftcard_id/width:$width", $CI->lang->line('common_edit'),array('class'=>'thickbox btn btn-small btn-info','title'=>$CI->lang->line($controller_name.'_update'))).'</td>';		
...
function get_item_kit_data_row($item_kit,$controller)
{
	...
	$table_data_row.='<td width="5%">'.anchor($controller_name."/view/$item_kit->item_kit_id/width:$width", $CI->lang->line('common_edit'),array('class'=>'thickbox btn btn-small btn-info','title'=>$CI->lang->line($controller_name.'_update'))).'</td>';		
