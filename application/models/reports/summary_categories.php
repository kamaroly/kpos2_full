...
class Summary_categories extends Report
{
	...
	{
		$this->db->select('cname as category, sum(subtotal) as subtotal, sum(total) as total, sum(tax) as tax, sum(profit) as profit');
		$this->db->from('sales_items_temp');
		$this->db->join('items', 'sales_items_temp.item_id = items.item_id');
		$this->db->join('items_categories', 'items_categories.id = items.category_id');
